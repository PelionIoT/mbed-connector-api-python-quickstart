...
@app.route('/')
def index():
    # get list of endpoints, for each endpoint get the pattern (/3201/0/5853)
    # value
...
def unsubscribeToPresses(data):
    ...
    e = connector.deleteResourceSubscription(
        data['endpointName'], '/3200/0/5501')
    while not e.isDone():
        None
    if e.error:
        print("Error: ", e.error.errType, e.error.error, e.raw_data)
    ...
    emit('unsubscribed-to-presses',
         {"endpointName": data['endpointName'], "value": 'True'})
